...
execute as @p[nbt={SelectedItem:{tag:{NDS_Wand:1b}}}] run function nightmare_skilltree:magic/items/wands/select_spell/check_default_damage_mainhand
# return on success
execute if entity @p[nbt={SelectedItem:{tag:{NDS_Wand:1b}}}] run return 0
# if not then make sure the player has offhand in hand
execute as @p[nbt={Inventory:[{Slot:-106b,tag:{NDS_Wand:1b}}]}] run function nightmare_skilltree:magic/items/wands/select_spell/check_default_damage_offhand
# return on success
